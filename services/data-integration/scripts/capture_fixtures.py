...
def capture(name: str, body: dict[str, object], token: str) -> dict[str, object]:
    ...
    if not isinstance(meta, dict):
        meta = {}
    counts = {key: len(data[key]) if isinstance(data.get(key), list) else 0 for key in RECORD_KEYS}
    ...
    degraded_mismatch = (
        any(outcome in {"UNAVAILABLE", "FAILED", "TIMED_OUT"} for outcome in outcomes.values())
        and not degraded
    )
    print(
        f"{name}: HTTP {status if status is not None else 'NO_RESPONSE'} "
        f"counts={json.dumps(counts, sort_keys=True)} degraded={degraded} "
        f"capabilities={json.dumps(outcomes, sort_keys=True)} "
        f"degraded_mismatch={degraded_mismatch}" + (f" error={error}" if error else ""),
        flush=True,
    )
    return {
        "fixture_id": name,
